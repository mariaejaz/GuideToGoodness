...
function handleTweets(data) {
  ...
  var html = '';
  var url = 'https://twitter.com/sheikhbabikir';

  while(n < x) {
   url = $($(data[n])[1].innerHTML).attr("href") + ' target="_blank"';

   html += '<li>' + '<p class="date">' + $(data[n])[1].innerHTML + '</p>' + 
   '<p>' + $(data[n])[0].innerHTML + '</p>' +
   ($(data[n])[2] !== undefined && $($(data[n])[2].innerHTML).attr("src") != 'undefined' ? '<a class="image" href='+ url +'>' + $(data[n])[2].innerHTML + '</a>' : '')  + '</li>';
   n++;
 }
...
function dateFormatter(date) {
 ...
 return monthNamesShort[date.getMonth()] +'<b>'+ date.getDate() +'</b>';
}
